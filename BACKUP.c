...
int mount(char *name)
{
	...
	printf("partition # of blocks: %d\n", PARTITION->block_size);
 	...
 	block_buffer = malloc(PARTITION->total_blocks*PARTITION->block_size);
...
{	// PURPOSE: assuming that the file is not already open, open it from fat, and assign to global pointer
	...
	if(isFileAlreadyOpen(name) != -1) // already opened, returned some pointer
	{
		return 0;
	}
...
int main()
{
	...
	writeResult = writeBlock(openResult, "tao");
